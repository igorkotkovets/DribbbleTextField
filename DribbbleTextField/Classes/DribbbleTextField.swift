...
class DribbbleTextField: UITextField {
    ...
    func setup() {
        ...
        beginEditingToken = NotificationCenter.default.addObserver(forName: NSNotification.Name.UITextFieldTextDidBeginEditing, object: self, queue: nil) { [weak self] (notification) in
            self?.applyEditingStateDesign()
        }

        endEditingToken = NotificationCenter.default.addObserver(forName: NSNotification.Name.UITextFieldTextDidEndEditing, object: self, queue: nil) { [weak self] (notification) in
            self?.applyEditingStateDesign()
        }
    }
